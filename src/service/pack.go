...
func NewPackService(repo repositories.PackageRepository) *Pack {
	return &Pack{
		repo: repo,
	}
}
...
func (p *Pack) CalculatePacks(order int, packages []int) (map[int]int, error) {
	if order <= 0 {
	...
	greedyCount := p.greedySolution(order, sortedPacks)
	upperBound := order
	if greedyCount > 0 {
		// Get tighter bound
...
func (p *Pack) greedySolution(order int, packs []int) int {
	count := 0
	remaining := order
